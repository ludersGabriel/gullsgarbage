...
Coming-soon landing page: black background, the gull, and "coming soon".
...
- `Dockerfile` — multi-stage: `base` (bun install), `builder` (`bun run build`),
  `runner` (`.output/`, `CMD bun .output/server/index.mjs`)
...
- **File-based routes** — `src/routes/__root.tsx` (full document shell:
  ...
  custom `notFoundComponent` gull 404) and `src/routes/index.tsx` (the
  coming-soon page). Route tree is generated to `src/routeTree.gen.ts` by
  the Vite plugin.
- **SSR mode per route** — the coming-soon page runs `ssr: true` (full SSR via
  the streaming `defaultStreamHandler`). Per-route SSR is configured with the
...
- **gullblock** — `src/resources/gullblock.png`, the blocky pixel-art gull
